...
function tree(breadcrumbs: Array<string>, blocks: Array<Block>): Tree {
	...
	const children: Array<string | undefined> = Array.from(
		new Set(descendants.map((block) => block.breadcrumbs[depth]))
	);
...
export function search(query: string): Array<Tree> {
	...
	const blocks = indices
		...
		.sort((a, b) => {
			...
			return (
				(a.block?.breadcrumbs.length ?? 0) - (b.block?.breadcrumbs.length ?? 0) ||
				(a?.rank ?? 0) - (b?.rank ?? 0)
			);
		})
		.map(({ block }) => block);
