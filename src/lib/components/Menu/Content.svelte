<script lang="ts">
	...
	import { NAVIGATION_KEYS, NAVIGATION_UP_KEYS, SUBMENU_CLOSE_KEYS } from './constants';
	...
	if ($menuStore.submenu && !hasContext(CONTEXT.SUBMENU)) {
		throw new Error('Menu.Content marked as a submenu must be used inside Menu.Sub');
	}
	...
	$: disableOutsidePointerEvents = $menuStore.open && $menuStore.modal && !$menuStore.submenu;
	$: disableOutsideScroll = $menuStore.modal && !$menuStore.submenu;

	function focusOutside(event: CustomEvent<FocusEvent>) {
		if (trapFocus || ($menuStore.submenu && $submenuStore.trigger === event.detail.target)) {
	...
	function keydown(event: KeyboardEvent) {
		...
		const inside = target.closest('[data-menu-content]') === event.currentTarget;
		...
		const intent: FocusIntent = NAVIGATION_UP_KEYS.includes(event.key) ? 'previous' : 'next';
