<script lang="ts">
  ...
  if (!hasContext(CONTEXT.MENUINDICATOR)) {
    throw new Error(
      "Menu.ItemIndicator must be used inside Menu.CheckItem or Menu.RadioItem"
    );
  }

  const indicatorStore: Writable<MenuItemIndicatorStoreType> = getContext(
    CONTEXT.MENUINDICATOR
  );
</script>
