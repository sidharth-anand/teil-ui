<script lang="ts">
	...
	$: percentage = (100 / ($sliderStore.max - $sliderStore.min)) * (value ?? 0 - $sliderStore.min);

	$: size = $sliderStore.orientation === 'horizontal' ? rect?.width : rect?.height;
	$: offset = size
		? thumbOffset(
				size,
				percentage,
				$sliderStore.movement === 'left' || $sliderStore.movement === 'top' ? 1 : -1
		  )
		: 0;
