<script lang="ts">
	import type { SliderStoreType, Direction, Orientation, SlideDirection } from './types';
	...
	function getNextSortedValues(prevValues: number[] = [], nextValue: number, index: number) {
	...
	function getValueFromPointer(position: number): number {
		...
		const output: [number, number] =
			slideDirection === 'left' || slideDirection === 'top' ? [min, max] : [max, min];
	...
	function update(value: number, index: number) {
		...
		const snap = roundValue(Math.round((value - min) / step) * step + min, decimal);
	...
	function slidemove(event: PointerEvent) {
		...
		update(getValueFromPointer(getPointerPosition(event)), $sliderStore.activeThumb);
	}
