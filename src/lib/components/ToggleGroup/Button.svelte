<script lang="ts">
	...
	if (!hasContext(CONTEXT.TOGGLEGROUP)) {
		throw new Error('ToggleGroup.Button must be used within a ToggleGroup.Group');
	}

	const toggleGroupStore = getContext<Writable<ToggleGroupStoreType>>(CONTEXT.TOGGLEGROUP);
