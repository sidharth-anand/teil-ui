<script lang="ts">
	...
	import type { ScrollStoreType, Orientation, Direction, ScrollbarSizes } from '../types';
	...
	function getThumbSize(size: ScrollbarSizes) {
		...
		const scrollbarPadding = size.scrollbar.paddingStart + size.scrollbar.paddingEnd;
	...
	function getThumbOffsetFromScroll(
		...
		const scrollClampRange =
			direction === 'ltr' ? [0, maxScrollPosition] : [maxScrollPosition * -1, 0];
		const scrollWithoutMomentum = clamp(scrollClampRange as [number, number], position);

		return scale([0, maxScrollPosition], [0, maxThumbPosition])(scrollWithoutMomentum);
	}
	...
	function getScrollPositionFromPointer(
		...
		const maxPointerPosition = size.scrollbar.size - sizes.scrollbar.paddingEnd - endOffset;
	...
	function thumbpositionchange() {
		if ($scrollStore.viewport && $scrollStore.thumb) {
			...
			$scrollStore.thumb.style.transform = `translate3d(${horizontal ? offset : 0}px, ${
				horizontal ? 0 : offset
			}px, 0)`;
		}
	}
...
<div
    ...
    right: {horizontal
		...
		? 0
		: undefined};
    --scroll-area-thumb-{horizontal ? 'width' : 'height'}: {getThumbSize(sizes)}px;
  "
