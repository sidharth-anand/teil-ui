<script lang="ts">
	...
	const debouncedScrollEnd = debounce(() => scrollStateMachine.step('SCROLL_END'), 100);

	$: scrollDirection = orientation === 'horizontal' ? 'scrollLeft' : 'scrollTop';
	...
	$: {
		if ($scrollStore.type === 'scroll' && $scrollState === 'idle') {
			hideTimer = setTimeout(() => scrollStateMachine.step('HIDE'), $scrollStore.delay);
		}
	}
</script>
