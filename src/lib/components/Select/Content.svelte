<script lang="ts">
  ...
  function vertical() {
    ...
    const itemOffsetMiddle =
      $selectStore.selected.offsetTop + selectedItemHalfHeight;
    ...
    if (willAlignWithoutTopOverflow) {
      const isLastItem =
        ...
        $focusStore.state.get($focusStore.items[$focusStore.items.length - 1])
          .node;
      ...
      const isFirstItem =
        $selectStore.selected ===
        $focusStore.state.get($focusStore.items[0]).node;
  ...
  function position() {
    ...
    if (!$selectStore.text || !$selectStore.content || $selectStore.positioned)
      return;
  ...
  function pointermove(event: PointerEvent) {
    delta = {
      x: Math.abs(
        Math.round(event.pageX) - $selectStore.triggerPointerPosition?.x ?? 0
      ),
      y: Math.abs(
        Math.round(event.pageY) - $selectStore.triggerPointerPosition?.y ?? 0
      ),
    };
  }
  ...
  $: {
    if (
      $selectStore.open &&
      $focusStore.currentStopIndex &&
      $selectStore.viewport
    ) {
      if ($focusStore.currentStopIndex === $focusStore.items[0]) {
        $selectStore.viewport.scrollTop = 0;
      } else if (
        $focusStore.currentStopIndex ===
        $focusStore.items[$focusStore.items.length - 1]
      ) {
...
{#if $selectStore.open}
  <div
    ...
    position: fixed;
    z-index: {$selectStore.content
      ? window.getComputedStyle($selectStore.content).zIndex
      : 0};
"
  >
    <div
